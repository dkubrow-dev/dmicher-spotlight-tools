...
Справка доступна мастеру и игрокам. Страницы сгруппированы по игровым действиям: дать слово, заметить молчащего игрока, проверить готовность, объявить перерыв. Раскрывайте разделы меню и меняйте его ширину перетаскиванием границы. Внизу всегда доступны автор, благодарности и Premium. В разделе настроек описаны параметры мастера, личных карточек, громкости и пороги аудита; значок вопроса возле поля показывает подсказку и открывает точное описание. Тексты и ссылки на русском и английском находятся в Spotlight, общий интерфейс справки — в Generics.

Help is available to GMs and players. Pages follow table actions: give someone a turn, notice a quiet participant, check readiness and announce a break. Expand navigation sections and drag the divider to resize the menu. Author, thanks and Premium remain at its bottom. Settings pages explain GM options, personal cards, volume and audit thresholds; each question icon shows a hint and opens the relevant description. Russian and English content and links remain in Spotlight; Generics provides the shared help interface.
...
Связь с Premium теперь проходит через `generics.premium` API 1. Spotlight объявляет две чистые синхронные точки расширения: `resolveConfiguration(stored, defaults)` для действующих параметров и `mergeConfiguration(stored, proposed)` для сохранения. Premium предоставляет их реализации в своём пакете Spotlight; обработчик получает `(base, ...args)` и может вызвать бесплатную реализацию. Generics отвечает за выбор совместимой реализации, уведомления, ограниченное ожидание запуска и переход к настройкам Premium. Проверка лицензии остаётся в Premium, а нормализация и правила настроек — в Spotlight. Отдельные функциональные модули не зависят друг от друга.

Spotlight изолирует входные данные расширения и принимает только предусмотренные платные поля. Отсутствие расширения, несовместимый контракт, отказ доступа или ошибка реализации возвращают бесплатное поведение; сохранённые платные значения остаются в мире. Подписка на изменения освобождается при закрытии страницы. Прямые `registerPremiumProvider`, `notifyPremiumChanged` и `premiumApiVersion` удалены из API Spotlight; Premium должен подключаться через Generics. Для этой рабочей пересборки обновите Generics и Premium вместе со Spotlight, поскольку номер версии сохранён.

Premium integration now uses `generics.premium` API 1. Spotlight declares two pure synchronous extension points: `resolveConfiguration(stored, defaults)` and `mergeConfiguration(stored, proposed)`. Premium implements them in its Spotlight feature package; handlers receive `(base, ...args)` and can delegate to the free implementation. Generics handles compatible dispatch, notifications, bounded startup waiting and the Premium settings entry point. License decisions stay in Premium; domain rules and normalization stay in Spotlight. Functional modules do not depend on one another.

Extension inputs are isolated and only declared paid fields are accepted. Missing, incompatible, denied or failing extensions preserve free operation and stored paid preferences. Subscriptions are disposed on page exit. The direct Spotlight APIs `registerPremiumProvider`, `notifyPremiumChanged` and `premiumApiVersion` have been removed; integrations must use Generics. Update Generics and Premium alongside this Spotlight development rebuild because version numbers have been retained.
